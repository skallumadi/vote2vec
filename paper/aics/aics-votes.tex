...
\end{tabu}
\vskip 1em
\caption{Group names, seats and ideology for the 6th, 7th parliamentary terms.}
\label{tbl:groups}
\end{table}

\subsection{Encoding Vote Data}
The EP plenary votes are publicly available and published regularly\footnote{\url{http://www.europarl.europa.eu/plenary/en/votes.html}}. Before applying techniques to roll call votes, we construct the vote matrix $X$: the high-dimensional representation of votes---where an entry contains a binary value for \emph{Yes}, \emph{No}, and optionally \emph{Abstain}, on each vote by an individual MEP. A small example representing this encoding for two roll call votes for three different MEPs is shown in Figure \ref{fig:votematrix}.

\begin{wrapfigure}{l}{0.25\textwidth}
\begin{center}
\vskip -2em
\includegraphics[width=0.25\textwidth]{figures/votematrix.pdf}
\end{center}
\vskip -1em
\caption{Example vote matrix: MEPs 1 and 2 voted \emph{No} on Vote 1, and abstained on Vote 2.}
...
\end{tabu}
\vskip 1em
\caption{WGSS: Within Group Scatter, votes from 6th Term. }
\label{tbl:6th}
\end{table}
...
\end{tabu}
\vskip 1em
\caption{BGSS - Between Group, votes from 6th Term.}
\end{table}
...
\end{tabu}
\vskip 1em
\caption{WGSS: Within Group Scatter, votes from 7th Term. }
\label{tbl:7th}
\end{table}
...
\end{tabu}
\vskip 1em
\caption{BGSS - Between Group Scatter, using votes from 7th Term. Larger values indicate greater separation between clusters of MEPs}
\end{table}

\section{Discussion}
While the methods we explore do not out perform the well established and widely used NOMINATE approach using a cluster validation based evaluation, there are a number of useful recommendations we can make when using different methods: NNDSVD initialization strategy for NMF produces most stable results; PCA initialization for t-SNE can help with stability of results. Even so, there is still a risk of over interpreting the structure that t-SNE produces. Before drawing any conclusions from visualisations made with t-SNE, we recommend paying particular attention to the implementation and parameters, especially the learning rate used during optimization. The SGNS approach allows most flexibility with arbitrary contexts, but is the least stable method.
...
We make the data for 6th and 7th EU parliaments and code to reproduce the approaches on different sets of voting records are made available at: \\ \url{https://github.com/igorbrigadir/vote2vec}.
